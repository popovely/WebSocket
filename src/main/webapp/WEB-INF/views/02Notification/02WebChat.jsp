...
window.onload = function() {
	...
	webSocket =
		new WebSocket('ws://localhost:8081/websocket/EchoServer.do');
	// 클라이언트가 접속했을때...
...
function wsMessage(event) {
	/*
		메세지를 |(파이프) 기호로 분리하여
		앞부분은 보낸사람, 뒷부분은 메세지로 각각 변수에 저장한다.
	*/
	...
	if(content == "") {
		// 전송된 내용이 없으면 아무것도 하지 않는다.
	}
	else {
		// 대화내용에 /가 포함되어있다면 명령어로 판단한다.
		if(content.match("/")) {
			/*
				현재 접속한 클라이언트의 아이디를 이용해서
				본인한테 온 귓속말인지 판단한다.
			...
			*/
			if(content.match(("/"+ chat_id))) {
				...
				messgaeWindow.scrollTop = messageWindow.scrollHeight;
			}
		}
		else {
			// 대화내용에 /가 없으면 일반적인 메세지로 판단한다.
