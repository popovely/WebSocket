...
@ServerEndpoint
	: 웹소켓 서버의 요청명을 지정한다.
	  주소는
	  	ws://localhost:8081/websocket/EchoServer.do
	  이와같이 설정한다.
...
public class WebSocketChat {
	/*
		해당 List컬렉션은 클라이언트가 접속할 때마다
		Session아이디를 저장하는 용도로 사용한다.
		접속한 웹브라우저가 웹소켓을 지원해야 하며,
	...
	/*
		자동으로 호출되진 않고,
		서버로 메세지가 왔을때 onMessage()에서 해당 메서드를 호출한다.
		현재 접속한 모든 클라이언트에게 메세지를 전송한다.
	 */
	private void sendAllSessionToMessage(Session self, String message) {
		try {
			// List컬렉션에 저장된 클라이언트 수 만큼 반복한다.
